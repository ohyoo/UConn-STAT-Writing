...
control tool today is `git`.:::example
** LaTeX vs. Word **

Suppose you want to write down a likelihood function for a Gaussian model:\[L(\mu, \sigma^2 \mid x_1, \ldots, x_n)
= (2\pi\sigma^2)^{
-n / 2
}\exp\left\{
-\frac{
...
-** In LaTeX ** , the source code is simple and transparent:```latex
L(\mu, \sigma^2 \mid x_1, \ldots, x_n) =
(2\pi\sigma^2)^{
-n / 2
}\exp\left\{
-\frac{
...
In MS Word, one must insert each symbol manually through the Equation Editor:Greek letters via dropdowns, summations via menus, exponents via special boxes.
Formatting quickly becomes clumsy, and editing dozens of formulas is tedious.
Copy - pasting often breaks structure, and version control is nearly impossible.

For one or two equations Word may be acceptable. But for an entire paper with dozens of equations, cross -
references, and theorems, LaTeX is the only tool that is professional, efficient, and sustainable.::::::{
.exercise
}
Compare Word and LaTeX

1. Typeset the following equation in ** MS Word ** using its built - in Equation Editor:\[\hat{
\beta
} = \arg\min_{
\beta \in \mathbb{
R
...
2n
}\ | y - X\beta\ | _2^2 + \lambda \ | \beta\ | _1 \right\
}.
\]

2. Now, paste this LaTeX code into Overleaf (or another LaTeX editor) and compile:```latex
\hat{
\beta
} = \arg\min_{
\beta \in \mathbb{
...
-Which is easier to revise and share ?:::While LaTeX is the professional standard for statistical and mathematical
...
### Set Up+Download Git [here](https: /  / git - scm.com / downloads).
